...
namespace DAL
{
    class UserInfo
    {
        ...
        public int GetCount()
        {
            using (IDbCornection cn = new MySqlCornection(cns))
            {
                string sql = "select count (1) from userinfo";
                return cn.Execut eScalar<int>(sql);
            }
        ...
        {
            using (IDbConnection cn = new MysqlCornection(cns))
            {
                string sql = "insert into userinfo values(@userName,@passWord,@gg, @email,@type, @userImg); ";
        ...
        public int update(Model.UserInfo user)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update userinfo set password=@passord gce=0g enail=@enail, type=时typ, usering-OuserIng where usernane=@userMane ";
                return cn.Execute(sql, user);
            }
        }
       public int Delete(string username)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = " delete from userinfo where username=@userMame";
